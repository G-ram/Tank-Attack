...
public class GamePadController
{
  ...
  */
  ...
  public double[] getRawXYStickDir()
  // return the (x,y) analog stick compass direction
  {
    if ((xAxisIdx == -1) || (yAxisIdx == -1)) {
      System.out.println("(x,y) axis data unavailable");
      ...
      return getRawCompassDir(xAxisIdx, yAxisIdx);
  }
  ...
  public double[] getRawZRZStickDir(){
    if ((zAxisIdx == -1) || (rzAxisIdx == -1)) {
      System.out.println("(z,rz) axis data unavailable");
      return null;
    }
    else 
      return getRawCompassDir(zAxisIdx, rzAxisIdx);
  }
  ...
  private double[] getRawCompassDir(int xA, int yA){
    float xCoord = comps[ xA ].getPollData();
    float yCoord = comps[ yA ].getPollData();
    double[] coords = {(double)xCoord,(double)yCoord};
    return coords;
  }
